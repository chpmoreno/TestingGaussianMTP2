MH <- function(K_ini, X, nMH, Gamma_sign = NULL, burn_rate = 0.1, plot_dens = TRUE,
                        statistics = TRUE) {
      S <- t(X) %*% X
      n <- nrow(X)
      p <- ncol(X)
      ...
      for(b in 2:(nMH + 1)) {
            ...
            for(i in 1:(p-1)) {
                  for(j in (i+1):p) {
                        if(Gamma_sign[i, j] == 0) {
                              K[i, i, b] <- rWISHART(1, 2 * n + p + 1, solve(as.matrix(S[i, i])))
                              K[j, j, b] <- rWISHART(1, 2 * n + p + 1, solve(as.matrix(S[j, j])))
                              K[i, j, b] <- 0
                              K[j, i, b] <- 0
                        } else{
                              R <- setdiff(1:p, c(i, j))
                              rhs <- -K[c(i, j), R, b] %*% solve(K[R, R, b]) %*% K[R, c(i, j), b]
                              tK <- rWISHART(1, n + p + 1, 2 * solve(diag(2) + S[c(i, j), c(i, j)]))
                              if(b == 2) {
                                    probK[ , 1] <- dWISHART(tK, 2 * n + p + 1, solve(diag(2) + S[c(i, j), c(i, j)]))
                                    tk_prev <- tK
                              } else {
                                    probK[ , 2] <- dWISHART(tK, 2 * n + p + 1, solve(diag(2) + S[c(i, j), c(i, j)]))
                                    prob_ratio <- min(1,  probK[ , 2] / probK[ , 1])
                                    probK[ , 1] <- probK[ , 2]
                                    if(prob_ratio == 1) {
                                          tK <- tK
                                    } else {
      ...
      return(list(K = K, Gibbs_summary = Gibbs_summary, 
                  ...
                  is_m_matrix_prop = is_m_matrix_prop))
}
